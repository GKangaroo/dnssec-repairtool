...
- DNSViz 类名：`MissingNSECForNODATA`
- 错误类别：`ResponseError`
- 当前状态：`done`
- 复现方法：`existing-lab`
- 复现场景：`missing-nsec-for-nodata`
...
```bash
python3 dnssec_lab.py demo missing-nsec-for-nodata
```

预期 DNSViz 错误码：

```text
MISSING_NSEC_FOR_NODATA, MISSING_RRSIG, MISSING_SEP_FOR_ALG, NO_SEP
```
