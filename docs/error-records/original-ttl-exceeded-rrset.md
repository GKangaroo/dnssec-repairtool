...
- DNSViz 类名：`OriginalTTLExceededRRset`
- 错误类别：`RRSIGError`
- 当前状态：`done`
- 复现方法：`existing-lab`
- 复现场景：`original-ttl-exceeded-rrset`
...
```bash
python3 dnssec_lab.py demo original-ttl-exceeded-rrset
```
...
重新签名子区并恢复 RRset TTL 不超过 RRSIG original TTL，重启 BIND 后复测。
